...
function float2rat(n, mathlook, tolerance) {
    ...
    if (mathlook == "none") {
	return '<span style="vertical-align:10px;">' + sign + ' </span><span class="frac"><sup>' + h1
...
var matrix = function(aRequests, nAugmented) {
	...
	if (oConfig.mathlook !== "none") {
		...
		sRowToDisplay = "";
		for (var nCol = 1; nCol <= nColCount; nCol++) {
		    sEntryToDisplay = float2rat(this.entries[nRow][nCol], oConfig.mathlook);
		...
		sRowToDisplay = sRowToDisplay.substring(0, sRowToDisplay.length - 1) + "\\\\ \r\n";
		...
		for (var nCol = 1; nCol <= nColCount; nCol++) {
		    sOutput += ("<td class='td-matrix'>" + float2rat(this.entries[nRow][nCol], oConfig.mathlook) + "</td>");
		}
		sOutput += "</tr>";
	    }
	    sOutput += "</table>"
	...
	// if not
		...
		return " Divide Row " + nRow + " by \\(" + float2rat(1 / nMultiplier) + "\\)"
	    }
	...
	if ((nRowToBeModified > nRowCount) || (nRowToBeMultiplied > nRowCount)) {
	    ...
	    for (nCol = 1; nCol <= nColCount; nCol++) {
		this.entries[nRowToBeModified][nCol] = parseFloat(this.entries[nRowToBeMultiplied][nCol] * nMultiplier)
		...
		if ((1 / nMultiplier) == Math.round(1 / nMultiplier)) {
		    return ("Divide Row " + nRowToBeMultiplied + " by \\(" + float2rat(-1 / nMultiplier)
			    + "\\) and substract from Row " + nRowToBeModified);
		} else {
		    return ("Multiply Row " + nRowToBeMultiplied + " by \\(" + float2rat(-nMultiplier)
			    + "\\) and substract from Row " + nRowToBeModified);
		}
	    } else {
		if ((1 / nMultiplier) == Math.round(1 / nMultiplier)) {
		    return ("Divide Row " + nRowToBeMultiplied + " by \\(" + float2rat(nMultiplier)
			    + "\\) and add to Row " + nRowToBeModified);
		} else {
		    return ("Multiply Row " + nRowToBeMultiplied + " by \\(" + float2rat(nMultiplier)
			    + "\\) and add to Row " + nRowToBeModified);
		}
	...
	while (aPivot != 0) {
	    ...
	    // sStepOutput = " + ")</div>";
	    sExplanation = "Found new pivot at entry (" + aNewPivot[0] + "," + aNewPivot[1] + ")";
		...
		nStepCount++;
		sNextStepID = "step" + nStepCount;
		writeOutputToElement(sNextStepID, this.interchangeRows(aNewPivot[0], aPivot[0] + 1), this.print({
		...
		nStepCount++;
		sNextStepID = "step" + nStepCount;
		writeOutputToElement(sNextStepID, this.multiplyRow(aPivot[0], nMultiplier), this.print({
		    pivot : aPivot,
		    mathlook : "display"
		}), "step");
	    }
		...
		// (this.entries[nRow][aPivot[1]]!=0))
		{
		    ...
		    nStepCount++;
		    sNextStepID = "step" + nStepCount;
		    writeOutputToElement(sNextStepID, this.addMultipleOfRow(row, aPivot[0], nMultiplier), this.print({
			pivot : aPivot,
			targetRow : row
		    }), "step");
		}
	...
	while (aPivot != 0) {
	    ...
	    nStepCount++;
	    sNextStepID = "step" + nStepCount;
	    writeOutputToElement(sNextStepID, "Found new pivot at entry (" + aNewPivot[0] + "," + aNewPivot[1] + ")",
		    this.print({
			pivot : aNewPivot
		    }), "step");
	...
	while (aPivot != 0) {
		...
		console.log(aPivot);
		writeOutputToElement(sNextStepID, this.interchangeRows(aNewPivot[0], aPivot[0] + 1), (sDet + this
			.print({
			    delimeter : "|",
			    pivot : [ aPivot[0] + 1, aNewPivot[1] ],
			    targetRow : aNewPivot[0],
		...
		writeOutputToElement(sNextStepID, this.multiplyRow(aPivot[0], nMultiplier), sDet + this.print({
		    delimeter : "|",
		    pivot : aPivot,
		    mathlook : "inline"
		}), "step");
		...
		// (this.entries[nRow][aPivot[1]]!=0))
		{
		    ...
		    nStepCount++;
		    sNextStepID = "step" + nStepCount;
		    writeOutputToElement(sNextStepID, this.addMultipleOfRow(row, aPivot[0], nMultiplier), sDet
			    + this.print	({
				delimeter : "|",
				pivot : aPivot,
				targetRow : row,
    ...
    this.inverse = function() {
	    ...
	    writeOutputToElement("augemented_matrix", "Augment original matrix by the identity matrix", augmentedMatrix
		    .print(), "step");
...
function scaleMath(element) {
    ...
    var w = math.width();
    if (w >= $(window).width()) {
	$(element).css("font-size", Math.floor($(window).width() / w * 95) + "%");
